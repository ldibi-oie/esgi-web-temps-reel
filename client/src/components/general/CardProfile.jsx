...
import { addFriend , addWaiting , auth, db} from '../../firebase/functions'
import { useAuthState } from 'react-firebase-hooks/auth';
import { query, collection, where, getDocs } from 'firebase/firestore';
export default function CardProfile({infos }) {
    ...
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-md purple:bg-gray-800 purple:border-gray-700 m-3">
        ...
        <div className="flex flex-col items-center pb-10">
            ...
            <div className="flex mt-4 space-x-3 md:mt-6">
                ...
                {
                    infos.roles === 'ADMIN_ROLES' && waitingList === undefined ?  (
                        <>
                            <button onClick={() => sendRequest(infos)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 purple:bg-blue-600 purple:hover:bg-blue-700 purple:focus:ring-blue-800">Demander a communiquer</button>
                        </>
                    ) : infos.roles === 'ADMIN_ROLES' && waitingList !== undefined  && waitingList.status === false ? (
                        <>
                            <button disabled className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 purple:bg-blue-600 purple:hover:bg-blue-700 purple:focus:ring-blue-800">Demande envoyée </button>
                        </>
                    ) : infos.roles === 'ADMIN_ROLES' && waitingList !== undefined   && waitingList.status === true ? (
                        <>
                            {/* <button onClick={() => addFriendFunction(infos)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 purple:bg-blue-600 purple:hover:bg-blue-700 purple:focus:ring-blue-800">Add friend</button> */}
                            <Link to={`/messenger/`} state={{ receiver: infos }} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 purple:bg-gray-800 purple:text-white purple:border-gray-600 purple:hover:bg-gray-700 purple:hover:border-gray-700 purple:focus:ring-gray-700">Message</Link>
                        </>
                    ) : infos.roles === 'ADMIN_ROLES' ?  (
                        <>
                            <button onClick={() => sendRequest(infos)} className="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 purple:bg-blue-600 purple:hover:bg-blue-700 purple:focus:ring-blue-800">Demander a communiquer</button>
                        </>
                    ) : ''
